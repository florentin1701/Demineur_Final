...
$(document).contextmenu(function() {return false;}); //TRES IMPORTANT permet de bloquer le clique droit !
$('#recommencer').attr("disabled", true); //On disable le bouton recommencer

//Function qui va valider la saisie de l'utilisateur
$('#creer').click(function validation_config() {
	lignes = $('#lignes').val();
...
$('#recommencer').click(function restart() {
...
function initialisation_game() {
  	...
  	score = 0;
  	$('#recommencer').attr("disabled", true);	//Le bouton recommencer est desactive. L'utilisateur doit perdre pour le reactiver ou gagner
  	...
  	$('#etat_game').html('Cliquez sur les cases pour commencer à Déminer').css('background-color','gray');
	 ...
	 for (var i = 0; i < tableau.length; i++) {
  	...
  	for (var ligne = 0; ligne < lignes; ligne++) {
    	for (var colonne = 0;  colonne < colonnes; colonne++) {
	      	...
	      	cases[ligne][colonne] = $("<img id=\""+id+"\" class=\"plan_demineur\"src=\"images/non_decouvert.png\">").css({"position": "absolute", "height": "30px", "width": "30px", "top": "" + position_top + "px", "left": "" + position_left + "px"});
	     	...
	     	//cases[ligne][colonne].addEventListener('mousedown', clique_jeu); //ANCIEN EN JS PURE CLIQUE JEU N'EXISTE PLUS
	      	//cases[ligne][colonne].classList.add("plan_demineur");
	      	...
	      	$("body").append(cases[ligne][colonne]);
    	}
  	}
  ...
  //Ici cette section permet de verifer le nombre qu'il a aux alentours et ainsi d'incrementer le nombre de chiffre 
	...
	$(".plan_demineur").mousedown(function(ev){
		var source = ev.target; //On recupere la source
  		var id = source.id;
  		var ligne = Math.floor(id / colonnes);
  		var colonne = id % colonnes;
  		if(jouable == true) {
			if (ev.which == 3) { //Si il clique droit
		      	switch (images[ligne][colonne]) {
		        	case 'non_decouvert': //Et que l'image est non decouvert
		          		cases[ligne][colonne].attr('src', 'images/drapeau.png'); //On remplace img par le drapeau
		        	...
		        	case 'drapeau': //idem ici si c'est un drapeau on met l'img question
		          		cases[ligne][colonne].attr('src', 'images/question.png');
		        	...
		        	case 'question': //Ici on revient au point de depart
		         		cases[ligne][colonne].attr('src', 'images/non_decouvert.png');
	  		...
	  		$('#etat_game').html('Mines restantes : ' + mines_restantes);
			if (ev.which == 1 && images[ligne][colonne] != 'drapeau') {
	    		if (tableau[ligne][colonne] == 'mine') {
	      			for (let ligne = 0; ligne < lignes; ligne++) {
	        			for (let colonne = 0; colonne < colonnes; colonne++) {
	          				if (tableau[ligne][colonne] == 'mine') {
	            				cases[ligne][colonne].attr('src', 'images/mine.png');
	          				}
	          				if (tableau[ligne][colonne] != 'mine' && images[ligne][colonne] == 'drapeau') {
	            				cases[ligne][colonne].attr('src', 'images/erreur_pas_de_mine_ici.png');
	         				}
	        			}
	        		}
				    score = nb_case_explore;
				    $('#etat_game').html('GAME OVER<br>Votre score :<br>Cases explorées : '+ score +'<br>Mine à chercher : '+ mines).css('background-color','#f92424');
				    $('#recommencer').attr("disabled", false);
				    jouable = false;
				    audio_gameover.play();
	    		} else {
	    			if (images[ligne][colonne] == 'non_decouvert') {
	    				reveler_case(ligne, colonne);
	    			}
	    		}
	  		}
	  		...
	  		if (mines_reveles == lignes * colonnes - mines) {
	      		...
	      		if((meilleur_score_case == null) || (meilleur_score_mine == null)) {
	        		...
	        		$('#etat_game').html('Mines Désarmorcées !<br>NOUVEAU RECORD : '+ score +' cases explorées<br>Mine à chercher : '+ mines).css('background-color','#6ff42c');
	      		} else {
	        		...
	        		$('#etat_game').html('Mines Désarmorcées !<br>Votre score : '+ score +' cases explorées<br>Mine à chercher : '+ mines +'<br>Meilleur Score : ' + meilleur_score_case + ' cases | ' + meilleur_score_mine + ' mines').css('background-color','#6ff42c');
	        		}
	      		}
	      		$('#recommencer').attr("disabled", false);
	      		bombe_desamorce();
...
function bombe_desamorce() {
  for (var ligne = 0; ligne < lignes; ligne++) {
    for (var colonne = 0;  colonne < colonnes; colonne++) { //On get les lignes et colonnes et on verif si dans le tableau on a une mine si oui on remplace l'image
      if(tableau[ligne][colonne] == "mine") {
        cases[ligne][colonne].attr('src', 'images/mine_desamorce.png');
      }
    }
  }
}
...
function reveler_case(ligne, colonne) {
  cases[ligne][colonne].attr('src', 'images/' + tableau[ligne][colonne] + '.png');
